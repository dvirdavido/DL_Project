...
#     parser.add_argument('-i', '--input', default='/data/ssd/ofersc/NNֹ_AEC/Test/real/',
#                         help='Path to directory containing enhanced nearend speech')
#     parser.add_argument('-d', '--dataset', default='/data/ssd/ofersc/NNֹ_AEC/Test/real/',
...
def aecmos(args, end_of_file):

    input_dir = args.path_wav
    dataset_dir = args.path_wav
    score_file = None

    clips = glob.glob(os.path.join(input_dir, '**', '*'+end_of_file +'.wav'), recursive=True)
    ...
    for clip_path in tqdm(clips):
        ...
        mic_path = clip_path[0:-len(end_of_file) -5 ] + '_mic.wav'
        lpb_path = clip_path[0:-len(end_of_file) -5 ] + '_lpb.wav'

        lpb_sig, mic_sig, enh_sig = read_and_process_audio_files(
            lpb_path, mic_path, clip_path)

        clip_scores = get_score(lpb_sig, mic_sig, enh_sig, 'doubletalk')
    ...
    return scores_df.echo_mos.mean(), scores_df.deg_mos.mean()
...
#     main(input_dir, dataset_dir, score_file)
    ...
    # python NNֹ_AEC/src/models/aecmos.py --i /data/ssd/ofersc/NNֹ_AEC/Test/real/ --d /data/ssd/ofersc/NNֹ_AEC/Test/real/